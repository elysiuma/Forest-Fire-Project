...
u8 GPS_get_time_and_pos(u8 *time, float *_position, char *_lati_longi_str)
{
    ...
    while (1)
    {
            ...
            (temp_rec[0] == '$' && temp_rec[4] == 0x48 && temp_rec[5] == 0x47 && temp_rec[6] == 0x41))
        {
            ...
            //printf("GPS Time: %d:%d:%d quality:%d, gps_num:%d\r\n", time[0], time[1], time[2],GPS_Quality,GPS_Satellite_Num);
            // 经纬度
            _position[0] = 10 * (temp_rec[18+start_idx] - '0') + (temp_rec[19+start_idx] - '0'); // 纬度
            strncpy(temp_pos_char, temp_rec + 20+start_idx, 8); // 纬分8位，格式为mm.mmmmm
            _position[1] = atof(temp_pos_char); // 纬度
            _lati_longi_str[0] = temp_rec[29+start_idx]; // 纬度标识
            _position[2] = 100 * (temp_rec[31+start_idx] - '0') + 10*(temp_rec[32+start_idx] - '0') + (temp_rec[33+start_idx] - '0'); // 经度
            strncpy(temp_pos_char, temp_rec + 34+start_idx, 8); // 经分8位，格式为mm.mmmmm
            _position[3] = atof(temp_pos_char); // 经度
            _lati_longi_str[1] = temp_rec[43+start_idx]; // 经度标识
