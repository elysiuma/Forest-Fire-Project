...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	{
		Res =USART_ReceiveData(USART1);//(UART4->DR);	//读取接收到的数据
		// printf("usart1: %02x\r\n", Res);
		//DebugLed();
		...
		{	
			...
			// 指令以6C开头则为LORA模块指令
			if((USART_RX_STA==0&&Res==0x6c)||USART_RX_BUF[0]==0X6C)
			{
				USART_RX_BUF[USART_RX_STA&0X3FFF]=Res;
				USART_RX_STA++;
				if((USART_RX_STA&0X3FFF)>=2)
				{
					Data_len = USART_RX_BUF[1] + 5; 
					//printf("USART_RX_BUF[1]: %i\r\n", USART_RX_BUF[1]); //Data_len = Res + 5;
					//printf("UART4_RX_STA: %i, Data len: %i", UART4_RX_STA, Data_len);
					if(USART_RX_STA>(USART_REC_LEN-1)) USART_RX_STA=0;//接收数据错误(超过最大接受字节数),重新开始接收
					if((USART_RX_STA&0X3FFF) == Data_len) USART_RX_STA|=0x8000;	//接收完成了(0x16为lora通信规约结束符)
				}
			}
			else{
				// 否则为普通指令
